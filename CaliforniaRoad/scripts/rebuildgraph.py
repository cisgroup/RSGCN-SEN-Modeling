...
def check_graph(graph):
    if (
        ...
        and sum(nx.get_edge_attributes(graph, "edge_attr").values()) > 5
    ):
        return True
    else:
        return False
...
if __name__ == "__main__":
    ...
    rebuild_G.add_edges_from(edge_longer, edge_attr=1.01)
    rebuild_G = connect_components(rebuild_G=rebuild_G, sorted_attr_ed=sorted_attr_ed)

    plot_a_sample(
        save_fig=True,
        filename=f"{dataset}/model_results/Orignal Map{model_name}.png",
        sample_graph=G,
        show_all_edges=True,
        threshold=0.5,
        pos=pos,
    ...
    plot_a_sample(
        save_fig=True,
        filename=f"{dataset}/model_results/Predicted Map{model_name}",
    ...
    hist_plot(
        ...
        f"Edge_length_Network{model_name}",
        model_name,
    )
    ...
    bar_plot(
        dataset, data_node, predict_node, f"Node degree_Network{model_name}", model_name
    )
