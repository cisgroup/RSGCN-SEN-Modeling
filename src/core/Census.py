...
def tract_data(year):
    census_file = "data/censusdata.pkl"
    ...
    if os.path.isfile(census_file):
        ...
        census_data = requests.get(
            f"https://api.census.gov/data/{year}/acs/acs5/profile?get={','.join(list(social_variable.keys()))}&for=tract:*&in=state:34,36,42&key={my_key}"
        )
